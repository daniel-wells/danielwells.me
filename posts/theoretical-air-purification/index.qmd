...
CADR can be measured according to the [ANSI/AHAM AC-1-2020](https://webstore.ansi.org/standards/aham/ansiahamac2020) standard.
...
What's the relationship between CADR and relative change in pollution over time?
...
We can define a mass balance equation:
...
So if the purification rate $\lambda_d$ is large compared to the combined effects of ventilation and deposition $(\lambda_v + \lambda_d)$ then the relative pollution is lower and vice versa.
...
However, it may be that the natural ventilation is somewhat less than 1 ACH and perhaps the linear fan speed to airflow correspondence I assumed is not quite correct due to the barrier of the purifier. Both of which could result in an improved relative reduction.
...
We could start to quantify this uncertianty by specifying a distribution for each of the input variables and using those to generate a distribution of plausible values of the relative reduction:
